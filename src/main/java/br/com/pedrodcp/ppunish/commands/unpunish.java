...
import static br.com.pedrodcp.ppunish.commands.checkpunish.autorName;
import static br.com.pedrodcp.ppunish.commands.checkpunish.keyID;

public class unpunish implements CommandExecutor {

    protected static String keyStringID;
    protected static int keyUnpunishId;
    ...
    public boolean onCommand(CommandSender s, Command c, String label, String[] args) {
        if (s instanceof Player) {
                ...
                if (!p.hasPermission("ppunish.unpunish")) {
                    ...
                    if (args.length >= 1) {
                        try {
                            ...
                            keyUnpunishId = keyIDLong.intValue();
                            for (Account accounts : Account.accounts) {
                                if (accounts.getID() == keyUnpunishId) {
                                    if (!args[0].equals("#0")) {
                                        if (!args[0].equals("0")) {
                                            for (Player all : Bukkit.getOnlinePlayers()) {
                                                keyID = String.valueOf(keyUnpunishId);
                                                autorName = p.getName();
                                                all.sendMessage("");
                                                ...
                                                new FancyMessage(" §e» ").text("§eAplicada §einicialmente §epor §e" + API.getAccount(accounts.getPlayerName()).getAutor()).hover("§7Clique §7para §7detalhes §7sobre §7a §7punição").command("/checkpunish #" + keyID).send(p);
                                                new FancyMessage(" §e» ").text("§eMotivo: §ePunição aplicada incorretamente.").hover("§7Clique §7para §7detalhes §7sobre §7a §7punição").command("/checkpunish #" + keyID).send(p);
                                                all.sendMessage("");
                                                p.sendMessage("§eA punição §b#" + accounts.getID() + " §edo jogador " + accounts.getPlayerName() + " foi revogada com sucesso.");
            ...
            if (c.getName().equalsIgnoreCase("unpunish")) {
                if (!s.hasPermission("ppunish.unpunish")) {
                                ...
                                if (accounts.getID() == keyUnpunishId) {
                                    if (!args[0].equals("#0")) {
                                        if (!args[0].equals("0")) {
                                            ...
                                            Bukkit.getConsoleSender().sendMessage("");
                                            s.sendMessage("§eA punição §b#" + accounts.getID() + " §edo jogador " + accounts.getPlayerName() + " foi revogada com sucesso.");
                                            for (PunishmentAccount punishmentAccounts : PunishmentAccount.accountsPunicoes) {
                                                if (punishmentAccounts.getID() == API.getAccount(accounts.getPlayerName()).getID()) {
